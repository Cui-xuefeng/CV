...
# 创建蓝图（用于注册到Flask应用）
detect_real_unet_bp = Blueprint("detect_real_unet_bp", __name__, url_prefix="/local")
...
@detect_real_unet_bp.route("/video_feed", methods=["GET"])
def video_feed():
    return Response(generate_mjpeg(), mimetype='multipart/x-mixed-replace; boundary=frame')

#启动检测接口
@detect_real_unet_bp.route("/detect", methods=["POST"])
def start_detect():
    try:
...
#停止检测接口
@detect_real_unet_bp.route("/stop_detect", methods=["POST"])
def stop_detect():
    with g.detect_lock:
        g.is_detecting = False
